...
void MDDSolver::solve_net_model(double & time_limit, ofstream& file1) {
	...
	int n_arcs_max = MDD_arcs[inst->n_SeatRow - 1].size();
	int n_nodes_max = MDD_nodes[inst->n_SeatRow - 1].size();

	cout << "n_arcs_max " << n_arcs_max << endl;
	cout << "n_nodes_max " << n_nodes_max << endl;

	file1 << n_nodes_max << ",";								//number of MDD nodes in one MDD
	file1 << n_arcs_max << ",";								//number of MDD arcs in one MDD
	...
	if (time_limit > PRECISION) {
		...
		try {
			...
			for (int s = 0; s < inst->n_seg; ++s) {
				...
				n_arcs = n_MDDarcs;
				n_nodes = MDD_nodes[inst->no_seat_in_seg[s] - 1].size();
				x[s] = new GRBVar[n_MDDarcs];
			...
			for (int s = 0; s < inst->n_seg; ++s) {
				...
				int n_MDDarcs = MDD_arcs[inst->no_seat_in_seg[s] - 1].size();
				for (int k = 1; k < n_MDDnodes; ++k) {
					GRBLinExpr equ = 0.0;
					for (int in = 0; in < MDD_nodes[inst->no_seat_in_seg[s] - 1][k]->input_arcs.size(); ++in) {
						equ += x[s][MDD_nodes[inst->no_seat_in_seg[s] - 1][k]->input_arcs[in]];
					}
					for (int out = 0; out < MDD_nodes[inst->no_seat_in_seg[s] - 1][k]->output_arcs.size(); ++out) {
						equ -= x[s][MDD_nodes[inst->no_seat_in_seg[s] - 1][k]->output_arcs[out]];
					}
			...
			for (int s = 0; s < inst->n_seg; s++) {
				int n_MDDarcs = MDD_arcs[inst->no_seat_in_seg[s] - 1].size();
				GRBLinExpr equ = 0.0;
				for (int out = 0; out < MDD_nodes[inst->no_seat_in_seg[s] - 1][0]->output_arcs.size(); ++out) {
					equ += x[s][MDD_nodes[inst->no_seat_in_seg[s] - 1][0]->output_arcs[out]];
				}
				...
				for (int nn = 0; nn < inst->max_party_size; ++nn) {
					if (inst->u[g][nn] > 0) {
						...
						for (int s = 0; s < inst->n_seg; ++s) {
							int n_MDDarcs = MDD_arcs[inst->no_seat_in_seg[s] - 1].size();
							for (int a = 0; a < n_MDDarcs; ++a) {
								if (MDD_arcs[inst->no_seat_in_seg[s] - 1][a]->size_group[0] == nn + 1 && MDD_arcs[inst->no_seat_in_seg[s] - 1][a]->size_group[1] == g) {
									party_equ += MDD_arcs[inst->no_seat_in_seg[s] - 1][a]->n_party_selected * x[s][a];
								}
							}
						}
			...
			for (int g = 0; g < inst->n_subGroup; ++g) {
				...
				for (int s = 0; s < inst->n_seg; ++s) {
					int n_MDDarcs = MDD_arcs[inst->no_seat_in_seg[s] - 1].size();
					for (int a = 0; a < n_MDDarcs; ++a) {
						if (MDD_arcs[inst->no_seat_in_seg[s] - 1][a]->size_group[1] == g) {
							LB_group_equ += MDD_arcs[inst->no_seat_in_seg[s] - 1][a]->value * x[s][a];
						}
					}
				}
				double new_LB = 0;
			...
			for (int s = 0; s < inst->n_seg; ++s) {
				int n_MDDarcs = MDD_arcs[inst->no_seat_in_seg[s] - 1].size();
				for (int a = 0; a < n_MDDarcs; ++a) {
					objective += MDD_arcs[inst->no_seat_in_seg[s] - 1][a]->value * x[s][a];
				}
			}

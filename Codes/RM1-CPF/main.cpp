...
int main() {
	...
	for (int instance = 1; instance < 11; ++instance) {
		...
		if (algo == 2) {
			...
			BDD_solver = new BDDSolver(inst);
			clock_t endTime = clock();
			time_limit -= (double(endTime - startTime) / (double)CLOCKS_PER_SEC);
			BDD_solver->solve_net_model(time_limit, file1);
		}
		...
		//file1 << inst->n_diff_item_size << ",";
		//file1 << inst->Tn_people / (inst->n_row * inst->n_SeatRow) << ",";
		//file1 << inst->Tn_party / inst->n_subGroup << endl;
	}

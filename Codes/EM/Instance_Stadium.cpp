...
void Stadium_Seating::read(const char* filename) {
	...
	// LB on the number of bins that can contain items of a specific size
	LB_itemSize.resize(max_party_size, 0);
	UB_itemSize.resize(max_party_size, n_seg);
	for (int nn = 0; nn < max_party_size; ++nn) {
		int min_bin = 0;
		int total_item = 0;
		for (int g = 0; g < n_subGroup; ++g) {
			total_item += u[g][nn];
		}
		UB_itemSize[nn] = min(n_seg, total_item);

		for (int b = 0; b < n_seg; ++b) {
			int total_weight = 0;
			int new_bin_item = 0;
			while (total_weight < n_SeatRow && total_item > 0) {
				total_weight += (nn + 1);
				new_bin_item++;
				total_item--;
			}
			...
			if (total_weight > n_SeatRow) {
				total_weight -= (nn + 1);
				new_bin_item--;
				total_item++;
			}
		}
		LB_itemSize[nn] = min_bin;
	}
	input.close();
